...
exports.rewardTaken=function(req,res)
{
	var value = req.param("button1");
	console.log(value);
	if (value == 1)
		{
res.render('congrats1', function (err,result)
		{
	if(!err)
			{
		res.end(result);
		...
		{
		res.render('congrats2', function (err,result)
				{
			if(!err)
					{
				res.end(result);
		...
		{     res.render('congrats3', function (err,result)
		{
	if(!err)
			{
		res.end(result);
...
{  
	res.render('thank you',function(err, result) {
		   // render on success
		   if (!err) {
		            res.end(result);
		   }
